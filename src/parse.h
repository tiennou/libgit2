...
#define git_parse_err(...) (giterr_set(GITERR_PATCH, __VA_ARGS__), -1)

#define git_parse_ctx_contains_s(ctx, str) \
	git_parse_ctx_contains(ctx, str, sizeof(str) - 1)

GIT_INLINE(bool)
git_parse_ctx_contains(git_parse_ctx *ctx, const char *str, size_t len)
{
...
int git_parse_advance_expected(
	git_parse_ctx *ctx, const char *expected, size_t expected_len);
...
enum GIT_PARSE_PEEK_FLAGS { GIT_PARSE_PEEK_SKIP_WHITESPACE = (1 << 0) };
