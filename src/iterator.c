...
GIT_INLINE(bool)
iterator_has_ended(git_iterator *iter, const char *path)
{
	if (iter->end == NULL)
		return false;
	else if (iter->ended)
	...
	git_iterator *iter, const char *path, size_t path_len)
{
	...
	while ((p = git_vector_get(&iter->pathlist, idx)) != NULL) {
		...
		if (p[path_len] == '/') {
			return (p[path_len + 1] == '\0') ? ITERATOR_PATHLIST_IS_DIR : ITERATOR_PATHLIST_IS_PARENT;
		}
...
GIT_INLINE(int)
tree_iterator_entry_cmp(const void *ptr_a, const void *ptr_b)
{
	...
	filesystem_iterator_entry *frame_entry)
{
	...
	if ((error = git_vector_init(&new_frame->entries, 64,
							iterator__ignore_case(&iter->base) ? filesystem_iterator_entry_cmp_icase : filesystem_iterator_entry_cmp)) < 0)
		goto done;
...
GIT_INLINE(void)
filesystem_iterator_frame_pop(filesystem_iterator *iter)
{
	filesystem_iterator_frame *frame;
...
GIT_INLINE(git_dir_flag)
entry_dir_flag(git_index_entry *entry)
{
#if defined(GIT_WIN32) && !defined(__MINGW32__)
	return (entry && entry->mode) ? (S_ISDIR(entry->mode) ? GIT_DIR_FLAG_TRUE : GIT_DIR_FLAG_FALSE) : GIT_DIR_FLAG_UNKNOWN;
#else
	GIT_UNUSED(entry);
	...
	git_iterator *i)
{
	...
	*status = GIT_ITERATOR_STATUS_NORMAL;
	 ...
	 * about.
	 */
	*status = current_entry->match == ITERATOR_PATHLIST_IS_PARENT ? GIT_ITERATOR_STATUS_FILTERED : GIT_ITERATOR_STATUS_EMPTY;
	...
	git_iterator_options *options)
{
	...
	*out = NULL;
	...
	iter->dirload_flags =
		...
		(iterator__flag(&iter->base, PRECOMPOSE_UNICODE) ? GIT_PATH_DIR_PRECOMPOSE_UNICODE : 0);
	...
	git_iterator_options *options)
{
	...
	*out = NULL;
	...
	git_vector_set_cmp(&iter->entries, iterator__ignore_case(&iter->base) ? git_index_entry_icmp : git_index_entry_cmp);
