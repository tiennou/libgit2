...
static int make_origin(git_blame__origin **out, git_commit *commit, const char *path)
{
	...
	if ((error = git_object_lookup_bypath(
			 &blob, (git_object *)commit, path, GIT_OBJ_BLOB)) < 0)
		return error;
...
int git_blame__get_origin(git_blame__origin **out, git_blame *blame,
	git_commit *commit, const char *path)
{
	git_blame__entry *e;
 ...
 */
static void blame_overlap(git_blame *blame, git_blame__entry *e, size_t tlno,
	size_t plno, size_t same, git_blame__origin *parent)
{
 ...
 */
static void blame_chunk(git_blame *blame, size_t tlno, size_t plno, size_t same,
	git_blame__origin *target, git_blame__origin *parent)
{
	git_blame__entry *e;
...
static int my_emit(long start_a, long count_a, long start_b, long count_b, void *cb_data)
{
...
static int diff_hunks(mmfile_t file_a, mmfile_t file_b, void *cb_data)
{
	...
	if (file_a.size > GIT_XDIFF_MAX_SIZE || file_b.size > GIT_XDIFF_MAX_SIZE) {
...
static int pass_blame_to_parent(
	git_blame *blame, git_blame__origin *target, git_blame__origin *parent)
{
...
static git_blame__origin *find_origin(
	git_blame *blame, git_commit *parent, git_blame__origin *origin)
{
	...
	if (!git_diff_num_deltas(difflist)) {
		...
		for (i = 0; i < (int)git_diff_num_deltas(difflist); i++) {
			...
			if (!git_vector_bsearch(NULL, &blame->paths, delta->new_file.path)) {
				git_vector_insert_sorted(&blame->paths,
					(void *)git__strdup(delta->old_file.path), paths_on_dup);
 ...
 */
static int pass_whole_blame(
	git_blame *blame, git_blame__origin *origin, git_blame__origin *porigin)
{
	git_blame__entry *e;
...
static int pass_blame(git_blame *blame, git_blame__origin *origin, uint32_t opt)
{
	...
	for (i = 0; i < num_parents; i++) {
		...
		for (j = same = 0; j < i; j++)
			if (sg_origin[j] &&
				!git_oid_cmp(git_blob_id(sg_origin[j]->blob),
...
static void coalesce(git_blame *blame)
{
	...
	for (ent = blame->ent; ent && (next = ent->next); ent = next) {
		if (same_suspect(ent->suspect, next->suspect) && ent->guilty == next->guilty &&
	...
	while (true) {
		...
		for (ent = blame->ent; ent; ent = ent->next) {
			if (same_suspect(ent->suspect, suspect)) {
				ent->guilty = true;
				ent->is_boundary = !git_oid_cmp(git_commit_id(suspect->commit),
