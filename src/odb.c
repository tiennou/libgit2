...
int git_odb__format_object_header(
	size_t *written, char *hdr, size_t hdr_size, git_off_t obj_len, git_otype obj_type)
{
...
static int init_fake_wstream(
	git_odb_stream **stream_p, git_odb_backend *backend, git_off_t size, git_otype type)
{
	fake_wstream *stream;
...
static int add_backend_internal(
	git_odb *odb, git_odb_backend *backend, int priority, bool is_alternate, ino_t disk_inode)
{
...
static int odb_exists_prefix_1(
	git_oid *out, git_odb *db, const git_oid *key, size_t len, bool only_refreshed)
{
	size_t i;
...
static int odb_read_header_1(
	size_t *len_p, git_otype *type_p, git_odb *db, const git_oid *id, bool only_refreshed)
{
	size_t i;
	git_otype ht;
...
static int read_prefix_1(
	git_odb_object **out, git_odb *db, const git_oid *key, size_t len, bool only_refreshed)
{
	size_t i;
	int error = 0;
...
int git_odb_open_rstream(
	git_odb_stream **stream, size_t *len, git_otype *type, git_odb *db, const git_oid *oid)
{
	size_t i, reads = 0;
	int error = GIT_ERROR;
...
int git_odb_write_pack(struct git_odb_writepack **out,
	git_odb *db,
