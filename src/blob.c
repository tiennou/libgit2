...
static int write_file_stream(git_oid *id, git_odb *odb, const char *path, git_off_t file_size)
{
	int fd, error;
...
static int write_file_filtered(git_oid *id,
	git_off_t *size,
	git_odb *odb,
...
int git_blob__create_from_paths(git_oid *id,
	struct stat *out_st,
	git_repository *repo,
...
int git_blob_filtered_content(git_buf *out, git_blob *blob, const char *path, int check_for_binary_data)
{
	int error = 0;
	git_filter_list *fl = NULL;
