...
GIT_INLINE(int) hash_cng_prov_init(void)
{
	...
	if ((dll_path_len = GetSystemDirectory(dll_path, MAX_PATH)) == 0 ||
		dll_path_len > MAX_PATH || StringCchCat(dll_path, MAX_PATH, "\\") < 0 ||
	...
	if ((hash_prov.prov.cng.open_algorithm_provider = (hash_win32_cng_open_algorithm_provider_fn)GetProcAddress(
			 hash_prov.prov.cng.dll, "BCryptOpenAlgorithmProvider")) == NULL ||
		(hash_prov.prov.cng.get_property = (hash_win32_cng_get_property_fn)GetProcAddress(
			 hash_prov.prov.cng.dll, "BCryptGetProperty")) == NULL ||
		(hash_prov.prov.cng.create_hash = (hash_win32_cng_create_hash_fn)GetProcAddress(
			 hash_prov.prov.cng.dll, "BCryptCreateHash")) == NULL ||
		(hash_prov.prov.cng.finish_hash = (hash_win32_cng_finish_hash_fn)GetProcAddress(
			 hash_prov.prov.cng.dll, "BCryptFinishHash")) == NULL ||
		(hash_prov.prov.cng.hash_data = (hash_win32_cng_hash_data_fn)GetProcAddress(
			 hash_prov.prov.cng.dll, "BCryptHashData")) == NULL ||
		(hash_prov.prov.cng.destroy_hash = (hash_win32_cng_destroy_hash_fn)GetProcAddress(
			 hash_prov.prov.cng.dll, "BCryptDestroyHash")) == NULL ||
		(hash_prov.prov.cng.close_algorithm_provider = (hash_win32_cng_close_algorithm_provider_fn)GetProcAddress(
			 hash_prov.prov.cng.dll, "BCryptCloseAlgorithmProvider")) == NULL) {
	...
	if (hash_prov.prov.cng.open_algorithm_provider(&hash_prov.prov.cng.handle,
			GIT_HASH_CNG_HASH_TYPE, NULL, GIT_HASH_CNG_HASH_REUSABLE) < 0) {
		FreeLibrary(hash_prov.prov.cng.dll);
	...
	if (hash_prov.prov.cng.get_property(hash_prov.prov.cng.handle,
			GIT_HASH_CNG_HASH_OBJECT_LEN, (PBYTE)&hash_prov.prov.cng.hash_object_size,
			sizeof(DWORD), &size_len, 0) < 0) {
...
{
	if (!CryptAcquireContext(&hash_prov.prov.cryptoapi.handle, NULL, 0,
...
GIT_INLINE(int) hash_cryptoapi_init(git_hash_ctx *ctx)
{
	...
	if (!CryptCreateHash(ctx->prov->prov.cryptoapi.handle, CALG_SHA1, 0, 0,
...
GIT_INLINE(int)
hash_cryptoapi_update(git_hash_ctx *ctx, const void *_data, size_t len)
{
...
GIT_INLINE(int) hash_cryptoapi_final(git_oid *out, git_hash_ctx *ctx)
{
	...
	if (!CryptGetHashParam(
			ctx->ctx.cryptoapi.hash_handle, HP_HASHVAL, out->id, &len, 0)) {
...
GIT_INLINE(int) hash_ctx_cng_init(git_hash_ctx *ctx)
{
	...
	if (hash_prov.prov.cng.create_hash(hash_prov.prov.cng.handle,
			&ctx->ctx.cng.hash_handle, ctx->ctx.cng.hash_object,
			hash_prov.prov.cng.hash_object_size, NULL, 0, 0) < 0) {
...
GIT_INLINE(int) hash_cng_init(git_hash_ctx *ctx)
{
	...
	if (ctx->prov->prov.cng.finish_hash(
			ctx->ctx.cng.hash_handle, hash, GIT_OID_RAWSZ, 0) < 0) {
...
GIT_INLINE(int)
hash_cng_update(git_hash_ctx *ctx, const void *_data, size_t len)
{
...
{
	if (ctx->prov->prov.cng.finish_hash(
			ctx->ctx.cng.hash_handle, out->id, GIT_OID_RAWSZ, 0) < 0) {
...
int git_hash_ctx_init(git_hash_ctx *ctx)
{
	...
	return (hash_prov.type == CNG) ? hash_ctx_cng_init(ctx) :
...
{
	assert(ctx && ctx->type);
	return (ctx->type == CNG) ? hash_cng_update(ctx, data, len) :
								hash_cryptoapi_update(ctx, data, len);
}

int git_hash_final(git_oid *out, git_hash_ctx *ctx)
{
	assert(ctx && ctx->type);
	return (ctx->type == CNG) ? hash_cng_final(out, ctx) :
								hash_cryptoapi_final(out, ctx);
}
