...
int git_fetchhead_ref_create(git_fetchhead_ref **out, git_oid *oid,
	unsigned int is_merge, const char *ref_name, const char *remote_url)
{
...
static int fetchhead_ref_write(git_filebuf *file, git_fetchhead_ref *fetchhead_ref)
{
	...
	} else if (git__prefixcmp(fetchhead_ref->ref_name, GIT_REFS_TAGS_DIR) == 0) {
		type = "tag ";
	...
	return git_filebuf_printf(file, "%s\t%s\t%s'%s' of %s\n", oid,
		(fetchhead_ref->is_merge) ? "" : "not-for-merge", type, name,
...
static int fetchhead_ref_parse(git_oid *oid, unsigned int *is_merge,
	git_buf *ref_name, const char **remote_url, char *line, size_t line_num)
{
	...
	*remote_url = NULL;
	...
	if (!*line) {
		giterr_set(GITERR_FETCHHEAD, "empty line in FETCH_HEAD line %" PRIuZ, line_num);
		return -1;
	}
	...
	if (git_oid_fromstr(oid, oid_str) < 0) {
		...
		giterr_set(GITERR_FETCHHEAD, "%s in FETCH_HEAD line %" PRIuZ, err_msg, line_num);
		return -1;
	}
...
int git_repository_fetchhead_foreach(
	git_repository *repo, git_repository_fetchhead_foreach_cb cb, void *payload)
{
