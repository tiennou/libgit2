	...
	const git_diff_delta *delta, float progress, void *data)
{
	...
	id_abbrev = delta->old_file.mode ? delta->old_file.id_abbrev : delta->new_file.id_abbrev;
	...
	git_buf_printf(
		out, (pi->id_strlen <= GIT_OID_HEXSZ) ? ":%06o %06o %s... %s... %c" : ":%06o %06o %s %s %c",
	...
	const git_diff_delta *delta, float progress, void *data)
{
	...
	if (binary && show_binary)
		id_strlen = delta->old_file.id_abbrev ? delta->old_file.id_abbrev : delta->new_file.id_abbrev;
