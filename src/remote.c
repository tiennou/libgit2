...
static int create_internal(
	git_remote **out, git_repository *repo, const char *name, const char *url, const char *fetch)
{
	git_remote *remote;
...
static int get_optional_config(
	bool *found, git_config *config, git_buf *buf, git_config_foreach_cb cb, void *payload)
{
	int error = 0;
...
static int set_url(
	git_repository *repo, const char *remote, const char *pattern, const char *url)
{
	git_config *cfg;
...
int git_remote_download(
	git_remote *remote, const git_strarray *refspecs, const git_fetch_options *opts)
{
	int error = -1;
	size_t i;
...
static int ref_to_update(
	int *update, git_buf *remote_name, git_remote *remote, git_refspec *spec, const char *ref_name)
{
	int error = 0;
	git_repository *repo;
...
static int update_tips_for_spec(git_remote *remote,
	const git_remote_callbacks *callbacks,
 ...
 */
static int next_head(const git_remote *remote,
	git_vector *refs,
	git_refspec **out_spec,
	git_remote_head **out_head,
	size_t *out_i,
	size_t *out_j,
	size_t *out_k)
{
...
static int rename_one_remote_reference(
	git_reference *reference_in, const char *old_remote_name, const char *new_remote_name)
{
	int error;
...
int git_remote_rename(
	git_strarray *out, git_repository *repo, const char *name, const char *new_name)
{
	int error;
...
int git_remote_upload(
	git_remote *remote, const git_strarray *refspecs, const git_push_options *opts)
{
	size_t i;
	int error;
	git_push *push;
