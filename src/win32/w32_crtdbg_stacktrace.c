...
static void *crtdbg__malloc(size_t len, const char *file, int line)
{
	void *ptr = _malloc_dbg(
		len, _NORMAL_BLOCK, git_win32__crtdbg_stacktrace(1, file), line);
	if (!ptr)
		giterr_set_oom();
	return ptr;
}
...
static void *crtdbg__calloc(size_t nelem, size_t elsize, const char *file, int line)
{
	void *ptr = _calloc_dbg(nelem, elsize, _NORMAL_BLOCK,
		git_win32__crtdbg_stacktrace(1, file), line);
	if (!ptr)
		giterr_set_oom();
	return ptr;
}
...
{
	char *ptr = _strdup_dbg(
		str, _NORMAL_BLOCK, git_win32__crtdbg_stacktrace(1, file), line);
	if (!ptr)
		giterr_set_oom();
	return ptr;
}
...
{
	void *new_ptr = _realloc_dbg(
		ptr, size, _NORMAL_BLOCK, git_win32__crtdbg_stacktrace(1, file), line);
	if (!new_ptr)
		giterr_set_oom();
	return new_ptr;
}

static void *crtdbg__reallocarray(
	void *ptr, size_t nelem, size_t elsize, const char *file, int line)
{
	...
	return GIT_MULTIPLY_SIZET_OVERFLOW(&newsize, nelem, elsize) ?
		NULL :
		_realloc_dbg(ptr, newsize, _NORMAL_BLOCK,
			git_win32__crtdbg_stacktrace(1, file), line);
}
	...
	const git_win32__stack__raw_data *pdata)
{
	...
	if (git__bsearch(g_cs_index, g_cs_ins, pdata, row_cmp, &pos) < 0) {
		...
		if (pos < g_cs_ins)
			memmove(&g_cs_index[pos + 1], &g_cs_index[pos],
...
static void dump_summary(const char *label)
{
	...
	if (g_limit_reached) {
		sprintf(buf, "LEAK SUMMARY: de-dup row table[%d] filled. Increase MY_ROW_LIMIT.\n",
			MY_ROW_LIMIT);
		my_output(buf);
	}
	...
	for (k = 0; k < g_cs_ins; k++) {
		if (g_cs_rows[k].transient_count_leaks > 0) {
			sprintf(buf, "LEAK: %s leaked %d of %d times:\n", g_cs_rows[k].uid.uid,
				g_cs_rows[k].transient_count_leaks, g_cs_rows[k].count_allocs);
			my_output(buf);

			if (git_win32__stack_format(buf, sizeof(buf),
					&g_cs_rows[k].raw_data, NULL, NULL) >= 0) {
				my_output(buf);
			}
...
int git_win32__crtdbg_stacktrace__dump(
	git_win32__crtdbg_stacktrace_options opt, const char *label)
{
...
#define IS_BIT_SET(o, b) (((o) & (b)) != 0)
	...
	bool b_leaks_since_mark = IS_BIT_SET(
		opt, GIT_WIN32__CRTDBG_STACKTRACE__LEAKS_SINCE_MARK);
...
void git_win32__crtdbg_stacktrace_cleanup(void)
{
	...
	git_win32__crtdbg_stacktrace__dump(
		GIT_WIN32__CRTDBG_STACKTRACE__LEAKS_TOTAL, "CLEANUP");
