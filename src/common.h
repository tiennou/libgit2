...
#define GITERR_CHECK_ALLOC_BUF(buf) \
	if ((void *)(buf) == NULL || git_buf_oom(buf)) { return -1; }
 ...
 */
GIT_INLINE(int)
giterr__check_version(const void *structure, unsigned int expected_max, const char *name)
{
	unsigned int actual;
...
#define GITERR_CHECK_VERSION(S, V, N) \
	if (giterr__check_version(S, V, N) < 0) return -1

/**
 * Initialize a structure with a version.
 */
GIT_INLINE(void)
git__init_structure(void *structure, size_t len, unsigned int version)
{
