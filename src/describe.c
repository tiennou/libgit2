...
static int replace_name(git_tag **tag,
	git_repository *repo,
...
static int add_to_known_names(git_repository *repo,
	git_oidmap *names,
	const char *path,
...
static int retrieve_peeled_tag_or_object_oid(
	git_oid *peeled_out, git_oid *ref_target_out, git_repository *repo, const char *refname)
{
	git_reference *ref;
...
static int show_suffix(
	git_buf *buf, int depth, git_repository *repo, const git_oid *id, size_t abbrev_size)
{
	int error, size = 0;
...
int git_describe_commit(
	git_describe_result **result, git_object *committish, git_describe_options *opts)
{
	struct get_name_data data;
	struct commit_name *name;
	git_commit *commit;
...
int git_describe_format(git_buf *out,
