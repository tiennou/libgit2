...
static int index_apply_to_wd_diff(git_index *index,
	int action,
...
static unsigned int index_merge_mode(git_index *index,
...
GIT_INLINE(int)
index_find_in_entries(size_t *out,
	git_vector *entries,
...
void git_index_entry__init_from_stat(git_index_entry *entry, struct stat *st, bool trust_mode)
{
 ...
 */
static int index_entry_create(git_index_entry **out,
	git_repository *repo,
	const char *path,
	struct stat *st,
	bool from_workdir)
...
static int index_entry_dup(git_index_entry **out,
	git_index *index,
...
static int index_entry_dup_nocache(git_index_entry **out,
	git_index *index,
	const git_index_entry *src)
{
	if (index_entry_create(out, INDEX_OWNER(index), src->path, NULL, false) < 0)
...
static int has_file_name(git_index *index, const git_index_entry *entry, size_t pos, int ok_to_replace)
{
	int retval = 0;
...
static int canonicalize_directory_path(git_index *index,
	git_index_entry *entry,
	git_index_entry *existing)
{
...
static void index_existing_and_best(git_index_entry **existing,
 ...
 */
static int index_insert(git_index *index,
...
int git_index__find_pos(size_t *out, git_index *index, const char *path, size_t path_len, int stage)
{
	assert(index && path);
...
int git_index_conflict_iterator_new(git_index_conflict_iterator **iterator_out,
	git_index *index)
{
...
int git_index_reuc_add(git_index *index,
	const char *path,
	int ancestor_mode,
	const git_oid *ancestor_oid,
	int our_mode,
...
static int read_entry(git_index_entry **out,
	size_t *out_size,
	git_index *index,
	const void *buffer,
	size_t buffer_size,
...
static int git_index_read_iterator(git_index *index,
...
int git_index_add_all(git_index *index,
	const git_strarray *paths,
	unsigned int flags,
	git_index_matched_path_cb cb,
	void *payload)
...
static int index_apply_to_all(git_index *index,
	int action,
	const git_strarray *paths,
	git_index_matched_path_cb cb,
	void *payload)
...
int git_index_remove_all(git_index *index,
...
int git_index_update_all(git_index *index,
	const git_strarray *pathspec,
	git_index_matched_path_cb cb,
	void *payload)
{
...
int git_index_snapshot_find(size_t *out,
	git_vector *entries,
	git_vector_cmp entry_srch,
	const char *path,
	size_t path_len,
