...
int git_merge_file__input_from_index(git_merge_file_input *input_out,
	git_odb_object **odb_object_out, git_odb *odb, const git_index_entry *entry)
{
	int error = 0;
...
static int merge_file__xdiff(git_merge_file_result *out,
	const git_merge_file_input *ancestor, const git_merge_file_input *ours,
	const git_merge_file_input *theirs, const git_merge_file_options *given_opts)
{
	xmparam_t xmparam;
	...
	if (ancestor) {
		xmparam.ancestor = (options.ancestor_label) ? options.ancestor_label :
													  ancestor->path;
		ancestor_mmfile.ptr = (char *)ancestor->ptr;
		ancestor_mmfile.size = ancestor->size;
	}

	xmparam.file1 = (options.our_label) ? options.our_label : ours->path;
	our_mmfile.ptr = (char *)ours->ptr;
	our_mmfile.size = ours->size;

	xmparam.file2 = (options.their_label) ? options.their_label : theirs->path;
	...
	if ((xdl_result = xdl_merge(&ancestor_mmfile, &our_mmfile, &their_mmfile,
			 &xmparam, &mmbuffer)) < 0) {
	...
	path = git_merge_file__best_path(
		ancestor ? ancestor->path : NULL, ours->path, theirs->path);
	...
	out->mode = git_merge_file__best_mode(
		ancestor ? ancestor->mode : 0, ours->mode, theirs->mode);
...
static int merge_file__binary(git_merge_file_result *out,
	const git_merge_file_input *ours, const git_merge_file_input *theirs,
...
static int merge_file__from_inputs(git_merge_file_result *out,
	const git_merge_file_input *ancestor, const git_merge_file_input *ours,
	const git_merge_file_input *theirs, const git_merge_file_options *given_opts)
{
	if (merge_file__is_binary(ancestor) || merge_file__is_binary(ours) ||
...
static git_merge_file_input *git_merge_file__normalize_inputs(
	git_merge_file_input *out, const git_merge_file_input *given)
{
...
int git_merge_file(git_merge_file_result *out,
	const git_merge_file_input *ancestor, const git_merge_file_input *ours,
	const git_merge_file_input *theirs, const git_merge_file_options *options)
{
...
int git_merge_file_from_index(git_merge_file_result *out, git_repository *repo,
	...
	const git_index_entry *theirs, const git_merge_file_options *options)
{
	git_merge_file_input *ancestor_ptr = NULL, ancestor_input = { 0 },
						 our_input = { 0 }, their_input = { 0 };
	git_odb *odb = NULL;
	...
	error = merge_file__from_inputs(out, ancestor_ptr, &our_input, &their_input, options);
