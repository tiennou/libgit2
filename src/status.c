...
static unsigned int workdir_delta2status(git_diff *diff, git_diff_delta *idx2wd)
{
	git_status_t st = GIT_STATUS_CURRENT;
	...
	case GIT_DELTA_RENAMED:
		...
		if (!git_oid_equal(&idx2wd->old_file.id, &idx2wd->new_file.id)) {
			...
			if (git_oid_iszero(&idx2wd->old_file.id) &&
				...
				!git_diff__oid_for_file(&idx2wd->old_file.id, diff,
			...
			if (git_oid_iszero(&idx2wd->new_file.id) &&
				...
				!git_diff__oid_for_file(&idx2wd->new_file.id, diff,
...
static bool status_is_included(
	git_status_list *status, git_diff_delta *head2idx, git_diff_delta *idx2wd)
{
	...
	if (idx2wd) {
		if (idx2wd->status != GIT_DELTA_ADDED && idx2wd->old_file.mode != GIT_FILEMODE_COMMIT)
			return 1;
...
static int status_collect(git_diff_delta *head2idx, git_diff_delta *idx2wd, void *payload)
{
...
status_entry_cmp_base(
	const void *a, const void *b, int (*strcomp)(const char *a, const char *b))
{
...
int git_status_list_new(
	git_status_list **out, git_repository *repo, const git_status_options *opts)
{
	...
	git_tree *head = NULL;
	git_status_show_t show = opts ? opts->show : GIT_STATUS_SHOW_INDEX_AND_WORKDIR;
	int error = 0;
	...
	*out = NULL;
	...
	if ((flags & GIT_STATUS_OPT_NO_REFRESH) == 0 && git_index_read(index, false) < 0)
		giterr_clear();
...
int git_status_foreach_ext(
	git_repository *repo, const git_status_options *opts, git_status_cb cb, void *payload)
{
...
int git_status_file(unsigned int *status_flags, git_repository *repo, const char *path)
{
	...
	opts.flags = GIT_STATUS_OPT_INCLUDE_IGNORED |
		...
		GIT_STATUS_OPT_INCLUDE_UNMODIFIED | GIT_STATUS_OPT_DISABLE_PATHSPEC_MATCH;
...
int git_status_list_get_perfdata(git_diff_perfdata *out, const git_status_list *status)
{
	assert(out);
