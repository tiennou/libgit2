...
int git_vector_insert(git_vector *v, void *element)
{
	...
	if (v->length >= v->_alloc_size && resize_vector(v, compute_new_size(v)) < 0)
		return -1;
...
int git_vector_bsearch2(
	size_t *at_pos, git_vector *v, git_vector_cmp key_lookup, const void *key)
{
...
int git_vector_search2(size_t *at_pos, const git_vector *v,
	git_vector_cmp key_lookup, const void *key)
{
	size_t i;
...
void git_vector_remove_matching(git_vector *v,
	int (*match)(const git_vector *v, size_t idx, void *payload), void *payload)
{
	size_t i, j;
...
int git_vector_resize_to(git_vector *v, size_t new_length)
{
	...
	if (new_length > v->length)
		memset(&v->contents[v->length], 0, sizeof(void *) * (new_length - v->length));
