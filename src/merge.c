...
int git_merge_bases_many(git_oidarray *out,
	git_repository *repo,
	size_t length,
	const git_oid input_array[])
{
...
int git_merge_base_octopus(git_oid *out,
	git_repository *repo,
	size_t length,
	const git_oid input_array[])
{
...
int git_merge_bases(git_oidarray *out,
	git_repository *repo,
	const git_oid *one,
	const git_oid *two)
{
...
static int paint_down_to_common(git_commit_list **out,
	git_revwalk *walk,
...
int git_merge__bases_many(git_commit_list **out,
	git_revwalk *walk,
	git_commit_list_node *one,
	git_vector *twos)
{
...
static int index_entry_similarity_calc(void **out,
	git_repository *repo,
...
static void merge_diff_list_count_candidates(git_merge_diff_list *diff_list,
	size_t *src_count,
	size_t *tgt_count)
{
...
GIT_INLINE(int)
merge_delta_type_from_index_entries(const git_index_entry *ancestor,
...
static git_merge_diff *merge_diff_from_index_entries(git_merge_diff_list *diff_list,
...
static int merge_diff_list_insert_unmodified(git_merge_diff_list *diff_list,
	const git_index_entry *tree_items[3])
{
	int error = 0;
	git_index_entry *entry;
...
static int merge_normalize_opts(git_repository *repo,
...
static int merge_index_insert_reuc(git_index *index, size_t idx, const git_index_entry *entry)
{
...
static int index_from_diff_list(git_index **out, git_merge_diff_list *diff_list, bool skip_reuc)
{
	git_index *index;
	size_t i;
	git_merge_diff *conflict;
...
static int write_merge_head(git_repository *repo,
...
static int write_merge_msg(git_repository *repo,
	const git_annotated_commit *heads[],
	size_t heads_len)
{
	git_filebuf file = GIT_FILEBUF_INIT;
...
static int merge_check_index(size_t *conflicts,
	git_repository *repo,
	git_index *index_new,
	git_vector *merged_paths)
{
...
static int merge_check_workdir(size_t *conflicts,
	git_repository *repo,
	git_index *index_new,
	git_vector *merged_paths)
{
