...
GIT_INLINE(int)
git_stream_connect(git_stream *st)
{
	return st->connect(st);
}

GIT_INLINE(int)
git_stream_is_encrypted(git_stream *st)
{
	return st->encrypted;
}

GIT_INLINE(int)
git_stream_certificate(git_cert **out, git_stream *st)
{
	if (!st->encrypted) {
...
GIT_INLINE(int)
git_stream_supports_proxy(git_stream *st)
{
...
GIT_INLINE(ssize_t)
git_stream_read(git_stream *st, void *data, size_t len)
{
...
GIT_INLINE(int)
git_stream_close(git_stream *st)
{
	return st->close(st);
}
