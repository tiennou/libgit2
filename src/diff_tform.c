...
GIT_INLINE(git_diff_file *)
similarity_get_file(git_diff *diff, size_t idx)
{
...
GIT_INLINE(bool)
delta_is_split(git_diff_delta *delta)
{
...
GIT_INLINE(bool)
delta_is_new_only(git_diff_delta *delta)
{
	...
	git_vector_foreach (&diff->deltas, t, tgt) {
		...
		if (src->status == GIT_DELTA_DELETED) {
			...
			if (delta_is_new_only(tgt)) {
				...
				src->status = (diff->new_src == GIT_ITERATOR_TYPE_WORKDIR) ? GIT_DELTA_UNTRACKED : GIT_DELTA_ADDED;
				src->nfiles = 1;
