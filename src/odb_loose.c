...
static int object_file_name(git_buf *name, const loose_backend *be, const git_oid *id)
{
	size_t alloclen;
...
{
	return git_futils_mkdir_relative(name->ptr + be->objects_dirlen,
		be->objects_dir, be->object_dir_mode,
...
static int parse_header(
	obj_hdr *out, size_t *out_len, const unsigned char *_data, size_t data_len)
{
	...
	*out_len = 0;
	...
	if (git__strntol64(&size, &data[size_idx], size_len, NULL, 10) < 0 || size < 0)
		goto on_error;
...
static int locate_object(
	git_buf *object_location, loose_backend *backend, const git_oid *oid)
{
	...
	if (git_path_isdir(pathbuf->ptr) == false) {
		...
		if (!memcmp(sstate->short_oid + 2,
			...
			if (!sstate->found) {
				...
				memcpy(sstate->res_oid + 2, pathbuf->ptr + sstate->dir_len,
					GIT_OID_HEXSZ - 2);
			}
			sstate->found++;
		}
...
static int locate_object_short_oid(git_buf *object_location, git_oid *res_oid,
	loose_backend *backend, const git_oid *short_oid, size_t len)
{
	...
	if (git_path_isdir(object_location->ptr) == false)
		return git_odb__error_notfound(
			"no matching loose object for prefix", short_oid, len);
	...
	error = git_path_direach(object_location, 0, fn_locate_object_short_oid, &state);
...
static int loose_backend__read_header(size_t *len_p, git_otype *type_p,
	git_odb_backend *backend, const git_oid *oid)
{
	...
	if (locate_object(&object_path, (loose_backend *)backend, oid) < 0) {
		error = git_odb__error_notfound("no matching loose object", oid, GIT_OID_HEXSZ);
...
static int loose_backend__read(void **buffer_p, size_t *len_p,
	git_otype *type_p, git_odb_backend *backend, const git_oid *oid)
{
	git_buf object_path = GIT_BUF_INIT;
	git_rawobj raw;
	int error = 0;
...
static int loose_backend__read_prefix(git_oid *out_oid, void **buffer_p,
	size_t *len_p, git_otype *type_p, git_odb_backend *backend,
...
static int loose_backend__foreach(
	git_odb_backend *_backend, git_odb_foreach_cb cb, void *data)
{
	char *objects_dir;
	int error;
...
static int loose_backend__writestream_finalize(
	git_odb_stream *_stream, const git_oid *oid)
{
	...
	if (object_file_name(&final_path, backend, oid) < 0 ||
		...
		error = git_filebuf_commit_at(&stream->fbuf, final_path.ptr);
...
static int loose_backend__writestream_write(
	git_odb_stream *_stream, const char *data, size_t len)
{
	loose_writestream *stream = (loose_writestream *)_stream;
...
static int loose_backend__writestream(git_odb_stream **stream_out,
	git_odb_backend *_backend, git_off_t length, git_otype type)
{
	...
	*stream_out = NULL;
	...
	if ((error = git_odb__format_object_header(
			 &hdrlen, hdr, sizeof(hdr), length, type)) < 0)
		return error;
...
static int loose_backend__readstream_read(
	git_odb_stream *_stream, char *buffer, size_t buffer_len)
{
...
static int loose_backend__readstream_packlike(obj_hdr *hdr, loose_readstream *stream)
{
	...
	return git_zstream_set_input(&stream->zstream, data + head_len, data_len - head_len);
}

static int loose_backend__readstream_standard(obj_hdr *hdr, loose_readstream *stream)
{
	unsigned char head[MAX_HEADER_LEN];
	size_t init, head_len;
	int error;

	if ((error = git_zstream_set_input(
			 &stream->zstream, stream->map.data, stream->map.len)) < 0)
		return error;
...
static int loose_backend__readstream(git_odb_stream **stream_out, size_t *len_out,
	git_otype *type_out, git_odb_backend *_backend, const git_oid *oid)
{
	loose_backend *backend;
...
static int loose_backend__write(git_odb_backend *_backend, const git_oid *oid,
	const void *data, size_t len, git_otype type)
{
	...
	if ((error = git_odb__format_object_header(
			 &header_len, header, sizeof(header), len, type)) < 0)
		goto cleanup;
...
static int loose_backend__freshen(git_odb_backend *_backend, const git_oid *oid)
{
	loose_backend *backend = (loose_backend *)_backend;
...
int git_odb_backend_loose(git_odb_backend **backend_out, const char *objects_dir,
	int compression_level, int do_fsync, unsigned int dir_mode, unsigned int file_mode)
{
	loose_backend *backend;
