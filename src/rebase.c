...
static int rebase_cleanup(git_rebase *rebase)
{
	...
	return git_path_isdir(rebase->state_path) ? git_futils_rmdir_r(rebase->state_path, NULL, GIT_RMDIR_REMOVE_FILES) : 0;
}
...
GIT_INLINE(int)
rebase_movenext(git_rebase *rebase)
{
...
int git_rebase_abort(git_rebase *rebase)
{
	...
	error = rebase->head_detached ? git_reference_create(&orig_head_ref, rebase->repo, GIT_HEAD_FILE,
...
static int notes_ref_lookup(git_buf *out, git_rebase *rebase)
{
	...
	error = do_rewrite ? git_config_get_string_buf(out, config, "notes.rewriteref") : GIT_ENOTFOUND;
