...
static int packfile_unpack_compressed(git_rawobj *obj, struct git_pack_file *p,
	git_mwindow **w_curs, git_off_t *curpos, size_t size, git_otype type);
 ...
 */
static int pack_entry_find_offset(git_off_t *offset_out, git_oid *found_oid,
	struct git_pack_file *p, const git_oid *short_oid, size_t len);
...
static void cache_free(git_pack_cache *cache)
{
	...
	if (cache->entries) {
		git_offmap_foreach_value (
			cache->entries, entry, { free_cache_object(entry); })
			;
...
static int cache_add(git_pack_cache_entry **cached_out, git_pack_cache *cache,
...
static unsigned char *pack_window_open(struct git_pack_file *p,
	git_mwindow **w_cursor, git_off_t offset, unsigned int *left)
{
...
static int packfile_unpack_header1(unsigned long *usedp, size_t *sizep,
	git_otype *type, const unsigned char *buf, unsigned long len)
{
	unsigned shift;
...
int git_packfile_unpack_header(size_t *size_p, git_otype *type_p,
	git_mwindow_file *mwf, git_mwindow **w_curs, git_off_t *curpos)
{
	unsigned char *base;
	unsigned int left;
	unsigned long used;
...
int git_packfile_resolve_header(size_t *size_p, git_otype *type_p,
	struct git_pack_file *p, git_off_t offset)
{
...
int git_packfile_unpack(git_rawobj *obj, struct git_pack_file *p, git_off_t *obj_offset)
{
	...
	error = pack_dependency_chain(
		&chain, &cached, obj_offset, small_stack, &stack_size, p, *obj_offset);
	if (error < 0)
		return error;
	...
	case GIT_OBJ_TAG:
		if (!cached) {
			...
			error = packfile_unpack_compressed(
				obj, p, &w_curs, &curpos, elem->size, elem->type);
			git_mwindow_close(&w_curs);
			base_type = elem->type;
		}
		if (error < 0)
	...
	while (elem_pos > 0 && !error) {
		...
		curpos = elem->offset;
		error = packfile_unpack_compressed(
			&delta, p, &w_curs, &curpos, elem->size, elem->type);
		git_mwindow_close(&w_curs);
		...
		error = git_delta_apply(
			&obj->data, &obj->len, base.data, base.len, delta.data, delta.len);
		obj->type = base_type;
...
int git_packfile_stream_open(
	git_packfile_stream *obj, struct git_pack_file *p, git_off_t curpos)
{
	int st;
		...
		*curpos += used;
		...
		if (p->has_cache) {
			...
			if (git_oidmap_valid_index(p->idx_cache, k)) {
				*curpos += 20;
				return ((struct git_pack_entry *)git_oidmap_value_at(p->idx_cache, k))
					->offset;
			} else {
		...
		if (pack_entry_find_offset(&base_offset, &unused, p,
				(git_oid *)base_info, GIT_OID_HEXSZ) < 0)
...
static int packfile_open(struct git_pack_file *p)
{
	...
	if (p_fstat(p->mwf.fd, &st) < 0 || git_mwindow_file_register(&p->mwf) < 0)
		goto cleanup;
	...
	if (p_read(p->mwf.fd, &hdr, sizeof(hdr)) < 0 ||
		hdr.hdr_signature != htonl(PACK_SIGNATURE) || !pack_version_ok(hdr.hdr_version))
		goto cleanup;
...
int git_pack_foreach_entry(struct git_pack_file *p, git_odb_foreach_cb cb, void *data)
{
	...
	struct git_pack_file *p, const git_oid *short_oid, size_t len)
{
	...
	*offset_out = 0;
	...
	printf("%02x%02x%02x... lo %u hi %u nr %d\n", short_oid->id[0],
		short_oid->id[1], short_oid->id[2], lo, hi, p->num_objects);
#endif
	...
	if (!found)
		return git_odb__error_notfound(
			"failed to find offset for pack entry", short_oid, len);
	if (found > 1)
