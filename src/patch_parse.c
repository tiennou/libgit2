...
typedef struct {
	 ...
	 * a rename (and rename paths are specified) or if no `+++`/`---` line
	 * specify the paths.
	 */
...
static int parse_header_git_oldpath(git_patch_parsed *patch, git_patch_parse_ctx *ctx)
{
...
static int parse_header_git_newpath(git_patch_parsed *patch, git_patch_parse_ctx *ctx)
{
...
static int parse_header_mode(uint16_t *mode, git_patch_parse_ctx *ctx)
{
	...
	if ((git_parse_advance_digit(&m, &ctx->parse_ctx, 8)) < 0)
		return git_parse_err(
			"invalid file mode at line %" PRIuZ, ctx->parse_ctx.line_num);
...
static int parse_header_oid(git_oid *oid, uint16_t *oid_len, git_patch_parse_ctx *ctx)
{
	size_t len;
...
static int parse_header_git_index(git_patch_parsed *patch, git_patch_parse_ctx *ctx)
{
	char c;
...
static int parse_header_git_oldmode(git_patch_parsed *patch, git_patch_parse_ctx *ctx)
{
	return parse_header_mode(&patch->base.delta->old_file.mode, ctx);
}

static int parse_header_git_newmode(git_patch_parsed *patch, git_patch_parse_ctx *ctx)
{
	return parse_header_mode(&patch->base.delta->new_file.mode, ctx);
}

static int parse_header_git_deletedfilemode(git_patch_parsed *patch,
	git_patch_parse_ctx *ctx)
{
...
static int parse_header_git_newfilemode(git_patch_parsed *patch,
	git_patch_parse_ctx *ctx)
{
...
static int parse_header_rename(char **out, git_patch_parse_ctx *ctx)
{
	git_buf path = GIT_BUF_INIT;
...
static int parse_header_renamefrom(git_patch_parsed *patch, git_patch_parse_ctx *ctx)
{
	patch->base.delta->status = GIT_DELTA_RENAMED;
	return parse_header_rename(&patch->rename_old_path, ctx);
}

static int parse_header_renameto(git_patch_parsed *patch, git_patch_parse_ctx *ctx)
{
	patch->base.delta->status = GIT_DELTA_RENAMED;
	return parse_header_rename(&patch->rename_new_path, ctx);
}

static int parse_header_copyfrom(git_patch_parsed *patch, git_patch_parse_ctx *ctx)
{
	patch->base.delta->status = GIT_DELTA_COPIED;
	return parse_header_rename(&patch->rename_old_path, ctx);
}

static int parse_header_copyto(git_patch_parsed *patch, git_patch_parse_ctx *ctx)
{
	patch->base.delta->status = GIT_DELTA_COPIED;
	return parse_header_rename(&patch->rename_new_path, ctx);
}
...
static int parse_header_similarity(git_patch_parsed *patch, git_patch_parse_ctx *ctx)
{
...
static int parse_header_dissimilarity(git_patch_parsed *patch, git_patch_parse_ctx *ctx)
{
...
static int parse_header_start(git_patch_parsed *patch, git_patch_parse_ctx *ctx)
{
	if (parse_header_path(&patch->header_old_path, ctx) < 0)
		return git_parse_err(
			"corrupt old path in git diff header at line %" PRIuZ,
			ctx->parse_ctx.line_num);

	if (git_parse_advance_ws(&ctx->parse_ctx) < 0 ||
		parse_header_path(&patch->header_new_path, ctx) < 0)
		return git_parse_err(
			"corrupt new path in git diff header at line %" PRIuZ,
			ctx->parse_ctx.line_num);
...
static int parse_header_git(git_patch_parsed *patch, git_patch_parse_ctx *ctx)
{
	size_t i;
	int error = 0;
	...
	for (; ctx->parse_ctx.remain_len > 0; git_parse_advance_line(&ctx->parse_ctx)) {
		...
		if (ctx->parse_ctx.line_len == 0 ||
			ctx->parse_ctx.line[ctx->parse_ctx.line_len - 1] != '\n')
			break;
		...
		for (i = 0; i < ARRAY_SIZE(transitions); i++) {
			...
			if (git_parse_advance_expected_str(&ctx->parse_ctx, "\n") < 0 ||
				...
				error = git_parse_err(
					"trailing data at line %" PRIuZ, ctx->parse_ctx.line_num);
				goto done;
			}
...
static int parse_hunk_header(git_patch_hunk *hunk, git_patch_parse_ctx *ctx)
{
...
static int parse_hunk_body(git_patch_parsed *patch,
	git_patch_hunk *hunk,
	git_patch_parse_ctx *ctx)
{
	git_diff_line *line;
	...
	for (; ctx->parse_ctx.remain_len > 1 && (oldlines || newlines) &&
		...
		default:
			error = git_parse_err(
				"invalid patch hunk at line %" PRIuZ, ctx->parse_ctx.line_num);
			goto done;
		}
...
static int parse_patch_header(git_patch_parsed *patch, git_patch_parse_ctx *ctx)
{
	int error = 0;
	...
	for (; ctx->parse_ctx.remain_len > 0; git_parse_advance_line(&ctx->parse_ctx)) {
		...
		if (git_parse_ctx_contains_s(&ctx->parse_ctx, "@@ -")) {
			...
			error = git_parse_err(
				"invalid hunk header outside patch at line %" PRIuZ, line_num);
			goto done;
		}
...
static int parse_patch_binary_side(git_diff_binary_file *binary,
	git_patch_parse_ctx *ctx)
{
	...
	if (git_parse_ctx_contains_s(&ctx->parse_ctx, "literal ")) {
		...
		error = git_parse_err("unknown binary delta type at line %" PRIuZ,
			ctx->parse_ctx.line_num);
		goto done;
	}
	...
	if (git_parse_advance_digit(&len, &ctx->parse_ctx, 10) < 0 ||
		...
		error = git_parse_err(
			"invalid binary size at line %" PRIuZ, ctx->parse_ctx.line_num);
		goto done;
	}
	...
	while (ctx->parse_ctx.line_len) {
		...
		if (!decoded_len) {
			error = git_parse_err("invalid binary length at line %" PRIuZ,
				ctx->parse_ctx.line_num);
			goto done;
		}
		...
		if (encoded_len > ctx->parse_ctx.line_len - 1) {
			error = git_parse_err("truncated binary data at line %" PRIuZ,
				ctx->parse_ctx.line_num);
			goto done;
		}
...
static int parse_patch_binary(git_patch_parsed *patch, git_patch_parse_ctx *ctx)
{
	int error;

	if (git_parse_advance_expected_str(&ctx->parse_ctx, "GIT binary patch") < 0 ||
		git_parse_advance_nl(&ctx->parse_ctx) < 0)
		return git_parse_err("corrupt git binary header at line %" PRIuZ,
			ctx->parse_ctx.line_num);

	/* parse old->new binary diff */
	if ((error = parse_patch_binary_side(&patch->base.binary.new_file, ctx)) < 0)
		return error;
	...
	if ((error = parse_patch_binary_side(&patch->base.binary.old_file, ctx)) < 0)
		return error;

	if (git_parse_advance_nl(&ctx->parse_ctx) < 0)
		return git_parse_err(
			"corrupt git binary patch separator at line %" PRIuZ,
			ctx->parse_ctx.line_num);
...
static int parse_patch_binary_nodata(git_patch_parsed *patch, git_patch_parse_ctx *ctx)
{
...
static int parse_patch_hunks(git_patch_parsed *patch, git_patch_parse_ctx *ctx)
{
	git_patch_hunk *hunk;
	int error = 0;
...
static int parse_patch_body(git_patch_parsed *patch, git_patch_parse_ctx *ctx)
{
...
int check_header_names(const char *one, const char *two, const char *old_or_new, bool two_null)
{
	if (!one || !two)
		return 0;
...
static int check_prefix(char **out, size_t *out_len, git_patch_parsed *patch, const char *path_start)
{
	...
	*out_len = 0;
	...
	if (remain_len || !*path)
		return git_parse_err(
			"header filename does not contain %" PRIuZ " path components", prefix_len);
...
static int check_patch(git_patch_parsed *patch)
{
	...
	if (delta->old_file.path && delta->status != GIT_DELTA_DELETED &&
		!delta->new_file.mode)
		delta->new_file.mode = delta->old_file.mode;

	if (delta->status == GIT_DELTA_MODIFIED && !(delta->flags & GIT_DIFF_FLAG_BINARY) &&
		delta->new_file.mode == delta->old_file.mode &&
...
git_patch_parse_ctx *git_patch_parse_ctx_init(const char *content,
	size_t content_len,
...
int git_patch_parse(git_patch **out, git_patch_parse_ctx *ctx)
{
	...
	*out = NULL;
	...
	if ((error = parse_patch_header(patch, ctx)) < 0 ||
		(error = parse_patch_body(patch, ctx)) < 0 || (error = check_patch(patch)) < 0)
		goto done;
...
int git_patch_from_buffer(git_patch **out,
	const char *content,
	size_t content_len,
	const git_patch_options *opts)
{
