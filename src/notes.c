	...
	int fanout)
{
	...
	*out = NULL;
	...
	for (i = 0; i < git_tree_entrycount(parent); i++) {
		...
		if (S_ISDIR(git_tree_entry_filemode(entry)) && strlen(git_tree_entry_name(entry)) == 2 && !strncmp(git_tree_entry_name(entry), annotated_object_sha + fanout, 2))
...
static int note_get_default_ref(char **out, git_repository *repo)
{
	...
	*out = (ret != 0) ? NULL : git_config__get_string_force(cfg, "core.notesref", GIT_NOTES_DEFAULT_REF);
