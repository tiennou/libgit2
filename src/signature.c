...
int git_signature_new(
	git_signature **sig_out, const char *name, const char *email, git_time_t time, int offset)
{
	...
	*sig_out = NULL;
	...
	if (contains_angle_brackets(name) || contains_angle_brackets(email)) {
	...
	char ender)
{
	...
	if (ender && (buffer_end = memchr(buffer, ender, buffer_end - buffer)) == NULL)
...
void git_signature__writebuf(git_buf *buf, const char *header, const git_signature *sig)
{
	...
	git_buf_printf(buf, "%s%s <%s> %u %c%02d%02d\n", header ? header : "",
		sig->name, sig->email, (unsigned)sig->when.time, sign, hours, mins);
}
...
bool git_signature__equal(const git_signature *one, const git_signature *two)
{
	...
	return git__strcmp(one->name, two->name) == 0 &&
		git__strcmp(one->email, two->email) == 0 && one->when.time == two->when.time &&
		one->when.offset == two->when.offset && one->when.sign == two->when.sign;
}
