...
static int ident_find_id(const char **id_start, const char **id_end, const char *start, size_t len)
{
...
static int ident_insert_id(git_buf *to, const git_buf *from, const git_filter_source *src)
{
	...
	need_size = (size_t)(id_start - from->ptr) + 5 /* "$Id: " */ +
		GIT_OID_HEXSZ + 2 /* " $" */ + (size_t)(from_end - id_end);
...
static int ident_remove_id(git_buf *to, const git_buf *from)
{
	...
	need_size = (size_t)(id_start - from->ptr) + 4 /* "$Id$" */ +
...
static int ident_apply(git_filter *self,
	void **payload,
	git_buf *to,
	const git_buf *from,
