...
#define DIFF_FLAG_SET(DIFF, FLAG, VAL) (DIFF)->base.opts.flags = \
																																								(VAL) ? ((DIFF)->base.opts.flags | (FLAG)) : ((DIFF)->base.opts.flags & ~(FLAG))
...
diff_delta__i2w_path(const git_diff_delta *delta)
{
	...
	const git_diff_options *opts)
{
	...
	if (diff->base.opts.ignore_submodules <= 0) {
		...
		if (entry && git_submodule_parse_ignore(&diff->base.opts.ignore_submodules, entry->value) < 0)
			giterr_clear();
	...
	diff_in_progress *info)
{
	...
	if (modified_uncertain && git_oid_iszero(&nitem->id)) {
		const git_oid *update_check =
			DIFF_FLAG_IS_SET(diff, GIT_DIFF_UPDATE_INDEX) && omode == nmode ? &oitem->id : NULL;
	...
	git_iterator *iterator)
{
	...
	while ((error = git_iterator_advance(entry, iterator)) == 0) {
		...
		cmp = (iterator->flags & GIT_ITERATOR_IGNORE_CASE) ? strcasecmp(prev_entry->path, (*entry)->path) : strcmp(prev_entry->path, (*entry)->path);
	...
	const git_diff_options *opts)
{
	...
	*out = NULL;
	...
	while (!error && (info.oitem || info.nitem)) {
		...
		cmp = info.oitem ? (info.nitem ? diff->base.entrycomp(info.oitem, info.nitem) : -1) : 1;
...
#define DIFF_FROM_ITERATORS(MAKE_FIRST, FLAGS_FIRST, MAKE_SECOND, FLAGS_SECOND) \
	do { \
		...
		char *pfx = (opts && !(opts->flags & GIT_DIFF_DISABLE_PATHSPEC_MATCH)) ? git_pathspec_prefix(&opts->pathspec) : NULL; \
