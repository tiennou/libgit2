...
#define GIT_SSL_DEFAULT_CIPHERS \
	"TLS-ECDHE-ECDSA-WITH-AES-128-GCM-SHA256:TLS-ECDHE-RSA-WITH-AES-128-GCM-" \
	"SHA256:TLS-ECDHE-ECDSA-WITH-AES-256-GCM-SHA384:TLS-ECDHE-RSA-WITH-AES-" \
	"256-GCM-SHA384:TLS-DHE-RSA-WITH-AES-128-GCM-SHA256:TLS-DHE-DSS-WITH-AES-" \
	"128-GCM-SHA256:TLS-DHE-RSA-WITH-AES-256-GCM-SHA384:TLS-DHE-DSS-WITH-AES-" \
	"256-GCM-SHA384:TLS-ECDHE-ECDSA-WITH-AES-128-CBC-SHA256:TLS-ECDHE-RSA-" \
	"WITH-AES-128-CBC-SHA256:TLS-ECDHE-ECDSA-WITH-AES-128-CBC-SHA:TLS-ECDHE-" \
	"RSA-WITH-AES-128-CBC-SHA:TLS-ECDHE-ECDSA-WITH-AES-256-CBC-SHA384:TLS-" \
	"ECDHE-RSA-WITH-AES-256-CBC-SHA384:TLS-ECDHE-ECDSA-WITH-AES-256-CBC-SHA:" \
	"TLS-ECDHE-RSA-WITH-AES-256-CBC-SHA:TLS-DHE-RSA-WITH-AES-128-CBC-SHA256:" \
	"TLS-DHE-RSA-WITH-AES-256-CBC-SHA256:TLS-DHE-RSA-WITH-AES-128-CBC-SHA:" \
	"TLS-DHE-RSA-WITH-AES-256-CBC-SHA:TLS-DHE-DSS-WITH-AES-128-CBC-SHA256:" \
	"TLS-DHE-DSS-WITH-AES-256-CBC-SHA256:TLS-DHE-DSS-WITH-AES-128-CBC-SHA:" \
	"TLS-DHE-DSS-WITH-AES-256-CBC-SHA:TLS-RSA-WITH-AES-128-GCM-SHA256:TLS-" \
	"RSA-WITH-AES-256-GCM-SHA384:TLS-RSA-WITH-AES-128-CBC-SHA256:TLS-RSA-" \
	"WITH-AES-256-CBC-SHA256:TLS-RSA-WITH-AES-128-CBC-SHA:TLS-RSA-WITH-AES-" \
	"256-CBC-SHA"
...
int git_mbedtls_stream_global_init(void)
{
	...
	if (mbedtls_ssl_config_defaults(git__ssl_conf, MBEDTLS_SSL_IS_CLIENT,
	...
	/* configure TLSv1 */
	mbedtls_ssl_conf_min_version(git__ssl_conf, MBEDTLS_SSL_MAJOR_VERSION_3,
	...
	if (mbedtls_ctr_drbg_seed(
			ctr_drbg, mbedtls_entropy_func, mbedtls_entropy, NULL, 0) != 0) {
	...
	if (!loaded && crtpath != NULL && stat(crtpath, &statbuf) == 0 &&
...
static int ssl_set_error(mbedtls_ssl_context *ssl, int error)
{
	...
	case MBEDTLS_ERR_X509_CERT_VERIFY_FAILED:
		giterr_set(GITERR_SSL, "SSL error: %#04x [%x] - %s", error,
			ssl->session_negotiate->verify_result, errbuf);
...
static int verify_server_cert(mbedtls_ssl_context *ssl)
{
	...
	if ((ret = mbedtls_ssl_get_verify_result(ssl)) != 0) {
		...
		giterr_set(GITERR_SSL, "the SSL certificate is invalid: %#04x - %s",
			ret, vrfy_buf);
...
ssize_t mbedtls_stream_write(git_stream *stream, const char *data, size_t len, int flags)
{
	...
	do {
		int error = mbedtls_ssl_write(
			st->ssl, (const unsigned char *)data + read, len - read);
		if (error <= 0) {
