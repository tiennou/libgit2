...
static int _apply_userpass_credential(
	HINTERNET request, DWORD target, DWORD scheme, git_cred *cred)
{
...
static int fallback_cred_acquire_cb(git_cred **cred, const char *url,
 ...
 * into the input.
 */
static int userpass_from_url(wchar_t **user, int *user_len, wchar_t **pass,
	int *pass_len, const wchar_t *url, int url_len)
{
...
static int parse_unauthorized_response(
	HINTERNET request, int *allowed_types, int *allowed_mechanisms)
{
...
static void CALLBACK winhttp_status(HINTERNET connection, DWORD_PTR ctx,
	DWORD code, LPVOID info, DWORD info_len)
{
	DWORD status;
...
static int winhttp_action(git_smart_subtransport_stream **stream,
	git_smart_subtransport *subtransport, const char *url, git_smart_service_t action)
{
