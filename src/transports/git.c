...
static int gen_proto(git_buf *request, const char *cmd, const char *url)
{
	...
	git_buf_printf(request, "%04x%s %s%c%s", (unsigned int)(len & 0x0FFFF), cmd,
		repo, 0, host);
...
static int git_proto_stream_read(git_smart_subtransport_stream *stream,
...
static int git_proto_stream_alloc(git_subtransport *t, const char *url,
	const char *cmd, const char *host, const char *port,
...
static int _git_uploadpack_ls(git_subtransport *t, const char *url,
	git_smart_subtransport_stream **stream)
{
	...
	*stream = NULL;
	...
	if ((error = gitno_extract_url_parts(
			 &host, &port, &path, &user, &pass, url, GIT_DEFAULT_PORT)) < 0)
		return error;
...
static int _git_uploadpack(git_subtransport *t, const char *url,
	git_smart_subtransport_stream **stream)
{
	GIT_UNUSED(url);
...
static int _git_receivepack_ls(git_subtransport *t, const char *url,
	git_smart_subtransport_stream **stream)
{
	char *host = NULL, *port = NULL, *path = NULL, *user = NULL, *pass = NULL;
	const char *stream_url = url;
...
static int _git_receivepack(git_subtransport *t, const char *url,
	git_smart_subtransport_stream **stream)
{
	GIT_UNUSED(url);
...
static int _git_action(git_smart_subtransport_stream **stream,
