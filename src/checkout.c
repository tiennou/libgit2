	...
	*action = CHECKOUT_ACTION__NONE;
	...
	case GIT_DELTA_MODIFIED: /* case 20 (or 37 but not really) */
		if (delta->old_file.mode == GIT_FILEMODE_COMMIT)
			...
			*action = git_iterator_current_is_ignored(workdir) ? CHECKOUT_ACTION_IF(DONT_OVERWRITE_IGNORED, CONFLICT, REMOVE_AND_UPDATE) : CHECKOUT_ACTION_IF(FORCE, REMOVE_AND_UPDATE, CONFLICT);
		break;
	...
	git_vector *pathspec)
{
	...
	while (1) {
		...
		if (cmp == 0) { /* case 5 */
			...
			return checkout_is_empty_dir(data, wd->path) ? checkout_action_with_wd_dir_empty(action, data, delta) : checkout_action_with_wd_dir(action, data, delta, workdir, wd);
		}
	...
	*actions_ptr = actions = git__calloc(
	...
	if (counts[CHECKOUT_ACTION__CONFLICT] > 0 &&
		...
		giterr_set(GITERR_CHECKOUT, "%" PRIuZ " %s checkout",
			counts[CHECKOUT_ACTION__CONFLICT],
			counts[CHECKOUT_ACTION__CONFLICT] == 1 ? "conflict prevents" : "conflicts prevent");
		error = GIT_ECONFLICT;
		goto fail;
	}
	...
	checkout_conflictdata *conflict)
{
	...
	opts.ancestor_label = data->opts.ancestor_label ? data->opts.ancestor_label : "ancestor";
...
static int checkout_create_conflicts(checkout_data *data)
{
	...
	git_vector_foreach (&data->update_conflicts, i, conflict) {
		...
		report_progress(data,
			conflict->ours ? conflict->ours->path : (conflict->theirs ? conflict->theirs->path : conflict->ancestor->path));
	}
	...
	const git_checkout_options *opts)
{
	...
	workdir_opts.flags = git_iterator_ignore_case(target) ? GIT_ITERATOR_IGNORE_CASE : GIT_ITERATOR_DONT_IGNORE_CASE;
	...
	baseline_opts.flags = git_iterator_ignore_case(target) ? GIT_ITERATOR_IGNORE_CASE : GIT_ITERATOR_DONT_IGNORE_CASE;
